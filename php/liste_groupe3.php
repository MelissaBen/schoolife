...
<?php 
echo'<head> <title>  Liste des étudiants du groupe 3</title> </head>';
 if (isset($_GET["edit"])){
                       if (isset($_GET["username"])){
                          $username=$_GET["username"];
                          #selectionner l'etudiant a modifier
                          $Sql = "SELECT * FROM e_groupe3 WHERE  username = '".$username."'  ";
                          ...
                          <div class="container" >
                                 ...
                                 <table id="mytable" class="table table-bordred table-striped">
                                     ...
                                     <tbody class="text-black">
                                      ...
                                      <tr>
                                      <form method="POST" action"index.php?page=listeGroupe3">
                       ...
                       if (isset($_GET["username"]))
                             $Sql = "SELECT * FROM e_groupe3 WHERE  username='".$_GET["username"]."'";
                             $Req = mysqli_query($result, $Sql);
                             $Resultat = mysqli_fetch_assoc($Req);
         ...
         if(isset($_POST["validdelete"])){
             $_GET["username"];
               $SQL= "DELETE FROM e_groupe3 WHERE username='".$_GET["username"]."'";
  ...
  $SQL= "UPDATE e_groupe3 SET   RaisonAb='".$RaisonAb."' WHERE username='".$username."'";
                        mysqli_query($result,$SQL) or die ("Error SQL : <br>".$SQL);
                      ...
                      if ($CountResultReq  > 0){  
                        while ($data = mysqli_fetch_array($ResultReq)){
                               echo'<td>'.$data["username"].'</td>
                                   ...
                                   <td>'.$data["email"].'</td>
                                   <td>'.$data["NbrAb"].'</td>
                                   <td>  <a href="index.php?page=listeGroupe3&edit&username='.$data["username"].'"><i class="fas fa-user-edit"></i></a> </td></td> 
                                   <td>  <a href="index.php?page=listeGroupe3&delete&username='.$data["username"].'"><i class="fas fa-user-minus"></i></a> </td>
                                  </tr>';
                       }
                      }else{
     ...
     echo'     <!--Le tableau  de la  liste des etudiants-->
          <div class="card shadow mb-4">
            <div  class="card-header py-3">
              <h6 class="m-0  font-weight-bold text-black" >Liste des etudiants du groupe 3 </h6>
            </div>
            <div class="card-body">
              ...
              <tr>';
                ...
                if ($countresult  > 0){  
                    while ($data = mysqli_fetch_array($Req)){
					  ...
					  <td>'.$data["RaisonAb"].'</td>
					   <td>'.$data["NbrRt"].'</td>
                       <td><a href="index.php?page=listeGroupe3&edit&username='.$data["username"].'"><i class="fas fa-edit"></i></a></td>
                       <td><a href="index.php?page=listeGroupe3&delete&username='.$data["username"].'"><i class="fas fa-trash"></i></a></td>
                       </tr>';
                    }
                 }
