...
<?php 
echo'<head> <title>   Liste des étudiants création  digitale</title> </head>';

 if (isset($_GET["edit"])){
                       if (isset($_GET["username"])){
                          $username=$_GET["username"];
                          #selectionner l'etudiant a modifier
                          $Sql = "SELECT * FROM e_creation_d WHERE  username = '".$username."'  ";
                          ...
                          <div class="container" >
                                 ...
                                 <table id="mytable" class="table table-bordred table-striped">
                                     ...
                                     <tbody class="text-black">
                                      ...
                                      <tr>
                                      <form method="POST" action"index.php?page=listeCrd">
                               ...
                               <td class="col-md-5"><input class="btn btn-primary col-md-12" id="btn" type="submit" name="validedit" value="enregistrer " >  </td>
                                  ...
                                  <td class="col-md-5"><a href="index.php?page=listeCrd"> <button id="btn-signup" class="btn btn-secondary " class="col-md-12">annuler </button> </a></td>
                                  </tr>
                                    </tbody>
                                </table>
                       ...
                       if (isset($_GET["username"]))
                             $Sql = "SELECT * FROM e_creation_d WHERE  username='".$_GET["username"]."'";
                             $Req = mysqli_query($result, $Sql);
                             $Resultat = mysqli_fetch_assoc($Req);
         ...
         if(isset($_POST["validdelete"])){
             $_GET["username"];
               $SQL= "DELETE FROM e_creation_d WHERE username='".$_GET["username"]."'";
  ...
  $SQL= "UPDATE e_creation_d SET   RaisonAb='".$RaisonAb."' WHERE username='".$username."'";
                        mysqli_query($result,$SQL) or die ("Error SQL : <br>".$SQL);
 ...
 echo'        
      <!-- BARRE DE RECHERCHE -->       
            <form name="frm" id="frm" class="form-inline" role="form" method="POST" action="index.php?page=listeCrD"> 
                      ...
                      if ($CountResultReq  > 0){  
                        while ($data = mysqli_fetch_array($ResultReq)){
                               echo'<td>'.$data["username"].'</td>
                                   ...
                                   <td>'.$data["email"].'</td>
                                   <td>'.$data["NbrAb"].'</td>
                                   <td>  </a><a href="index.php?page=listeCrD&edit&username='.$data["username"].'"><i class="fas fa-user-edit"></i></a> </td></td> 
                                   <td>  </a><a href="index.php?page=listeCrD&delete&username='.$data["username"].'"><i class="fas fa-user-minus"></i></a> </td> 
                                  </tr> ';
                       }
                      }else{
                             echo ' <tr> <td colspan="7">Aucun étudiant  trouvée </td>   </tr>
                                     ';
                           }
                           echo "  </tbody>
                              </table>
     ...
     echo'     <!--Le tableau  de la  liste des etudiants-->
          <div class="card shadow mb-4">
            <div  class="card-header py-3">
              <h6 class="m-0  font-weight-bold text-black" >Liste des étudiant création digitale </h6>
            </div>
            <div class="card-body">
              ...
              <tr>';
                ...
                if ($countresult  > 0){  
                    while ($data = mysqli_fetch_array($Req)){
					  ...
					  <td>'.$data["RaisonAb"].'</td>
					   <td>'.$data["NbrRt"].'</td>
                       <td><a href="index.php?page=listeCrd&edit&username='.$data["username"].'"><i class="fas fa-edit"></i></a></td>
                       <td><a href="index.php?page=listeCrd&delete&username='.$data["username"].'"><i class="fas fa-trash"></i></a></td>
                       </tr>';
                    }
                 }
